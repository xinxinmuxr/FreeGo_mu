...
public class QueryLike {
    ...
    public static List<Integer> likeUserSix(int userId){
        ...
        for(int i = 0;i < userNowPrefer.size();i++){
            NowUserGenfang += Math.sqrt(userNowPrefer.get(i).getPreferWeight()*userNowPrefer.get(i).getPreferWeight());
        }
        //计算登录用户与其他用户的
        for(int i = 0;i < userList.size();i++){
            ...
            for(int l = 0;l < userNowPrefer.size();l++){
                OtherUserGenfang += Math.sqrt(userOtherPrefer.get(l).getPreferWeight()*userOtherPrefer.get(l).getPreferWeight());
            }
    ...
    public static void main(String[] args) throws ParseException {
        //updateImg();
        //HotelInfoDaoImpl imp = new HotelInfoDaoImpl();
        //imp.queryHotelInPicture(1000);
        /*List<Integer> userListSix = new ArrayList<Integer>();  //选出的6个人
        userListSix = likeUserSix(1055);*/
        queryLike(1002);
    }
}
